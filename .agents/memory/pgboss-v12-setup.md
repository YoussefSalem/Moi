...
6. **`WorkHandler` delivers `JobWithMetadata<T>[]`, not `Job<T>[]`** — use `import type { JobWithMetadata } from "pg-boss"` for the handler argument type; `retryCount` (camelCase) lives on `JobWithMetadata`, not the base `Job`.
