...
export function ProductPage({ handle, onBack, onNavigate, onPageNavigate }: ProductPageProps) {
  ...
  return (
    <>
      ...
      {/* Size Guide Modal — portaled to document.body so it centres on the true viewport */}
      {createPortal(
        <AnimatePresence>
          {sizeGuideOpen && (
            <motion.div
            ...
            >
              <motion.div
                ...
                initial={isMobileScreen ? { y: "100%", opacity: 1 } : { scale: 0.95, opacity: 0 }}
                animate={isMobileScreen ? { y: 0, opacity: 1 } : { scale: 1, opacity: 1 }}
                exit={isMobileScreen ? { y: "100%", opacity: 1 } : { scale: 0.95, opacity: 0 }}
                transition={{ duration: 0.3, ease: [0.22, 1, 0.36, 1] }}
                onClick={(e) => e.stopPropagation()}
                style={{
                  background: "#faf8f5",
                  width: "100%",
                  maxWidth: isMobileScreen ? "100%" : 540,
                  borderRadius: isMobileScreen ? "16px 16px 0 0" : "12px",
                  maxHeight: "90dvh",
                  display: "flex",
              ...
              >
                ...
                {/* Scrollable body */}
                <div style={{ overflowY: "auto", flex: 1, padding: "20px 24px", paddingBottom: "max(env(safe-area-inset-bottom), 24px)", WebkitOverflowScrolling: "touch" } as React.CSSProperties}>
                  ...
                  {/* How to measure tip */}
                  <div style={{ marginTop: 24, marginBottom: 8, padding: "14px 16px", border: "1px solid rgba(30,24,20,0.10)", borderRadius: 6 }}>
