...
export function ProductCard({ product, onLookView }: ProductCardProps) {
  ...
  // Effective product passed to LookView — carries the resolved, color-matched look
  const effectiveProduct: ProductConfig = {
    ...product,
    look: effectiveLook,
  };

  // Ambient gradient driven by the current color's hero shot
  const color = useImageColor(effectiveProductShot);
  ...
  function getDefaultStock(color: string, size: string): number {
    ...
    const colorInventory = product.defaultInventory[color.toLowerCase()];
  ...
  useEffect(() => {
    ...
    const sizeExistsForColor = product.variants.some((v) =>
      v.selectedOptions.some(
        (o) => o.name.toLowerCase() === (sizeOption.optionName.toLowerCase() ?? "size") && o.value === selectedSize
      ) &&
      (!colorOption || v.selectedOptions.some(
        (o) => o.name.toLowerCase() === "color" && o.value === selectedColor
      ))
    );
    if (!sizeExistsForColor) {
      const firstExisting = sizeOption.values.find((s) =>
        product.variants!.some((v) =>
          v.selectedOptions.some(
            (o) => o.name.toLowerCase() === (sizeOption.optionName.toLowerCase() ?? "size") && o.value === s
          ) &&
          (!colorOption || v.selectedOptions.some(
            (o) => o.name.toLowerCase() === "color" && o.value === selectedColor
          ))
    ...
    : product.variants
      ? [...new Set(product.variants.flatMap((v) =>
          v.selectedOptions
            ...
            .map((o) => o.value)
        ))].map((name) => ({ name, swatch: swatchFor(name) }))
      : [];
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Center: color-specific product shot — crossfades when color changes */}
          <div className="flex justify-center relative">
            <div className="relative">
              ...
              >
                <motion.img
                  key={effectiveProductShot}
                  src={effectiveProductShot}
                  alt={product.name}
        ...
        {/* Per-color filmstrip carousel — updates when selected color changes */}
        <Carousel filmstrip={effectiveFilmstrip} />
      </section>
