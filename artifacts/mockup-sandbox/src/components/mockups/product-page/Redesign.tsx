...
function PlusIcon({ open }: { open: boolean }) {
  return (
    <span style={{ display: "inline-block", width: 16, height: 16, position: "relative", flexShrink: 0, transition: "transform 0.25s ease", transform: open ? "rotate(45deg)" : "rotate(0deg)" }}>
    ...
    <div style={{ minHeight: "100vh", background: "#faf8f5", fontFamily: "'Montserrat', sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 768px) {
          ...
          .left-col  { order: 2; padding: 24px 0 0 !important; }
          .center-col { order: 1; }
          .right-col { order: 3; padding: 24px 0 0 !important; }
          .header-inner { padding: 0 20px !important; }
          .breadcrumb   { padding: 12px 20px !important; }
          .heading { font-size: 38px !important; }
        }
      `}</style>
      ...
      <div className="product-layout">
        ...
        <div className="left-col">
          ...
          <h1 className="heading" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 46, fontWeight: 400, lineHeight: 1.05, color: "#1a1410", marginBottom: 8, letterSpacing: "0.01em" }}>
            MOI<br />WAVVY
          </h1>
            ...
            {ACCORDION.map(item => (
              <div key={item.key} style={{ borderBottom: "1px solid rgba(26,20,16,0.1)" }}>
                ...
                {openAccordion === item.key && (
                  <p style={{ fontSize: 12, color: "rgba(26,20,16,0.6)", lineHeight: 1.85, letterSpacing: "0.025em", paddingBottom: 16 }}>{item.body}</p>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="center-col" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ position: "relative", background: "#f0ece6", overflow: "hidden" }}>
            ...
            <div style={{ position: "absolute", bottom: 14, left: "50%", transform: "translateX(-50%)", display: "flex", gap: 6 }}>
              {images.map((_, i) => (
                <button key={i} onClick={() => setGalleryIdx(i)} style={{ width: i === galleryIdx ? 18 : 6, height: 6, borderRadius: 3, background: i === galleryIdx ? "#1a1410" : "rgba(26,20,16,0.3)", transition: "all 0.25s", border: "none", padding: 0 }} />
              ))}
            </div>
          </div>
        ...
        <div className="right-col">
          ...
          <div style={{ marginBottom: 28 }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {COLORS.map((c, i) => (
                <button key={c.name} onClick={() => handleColorSelect(i)} title={c.name} style={{ width: 28, height: 28, borderRadius: "50%", background: c.swatch, border: i === selectedColor ? "2px solid #1a1410" : "2px solid transparent", outline: i === selectedColor ? "none" : "1px solid rgba(26,20,16,0.18)", outlineOffset: 2, transition: "border 0.2s, transform 0.15s", transform: i === selectedColor ? "scale(1.15)" : "scale(1)" }} />
              ))}
            </div>
          </div>
          ...
          <div style={{ marginBottom: 32 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {SIZES.map(size => (
                <button key={size} onClick={() => setSelectedSize(size)} style={{ padding: "10px 16px", fontSize: 10, letterSpacing: "0.16em", textTransform: "uppercase", color: selectedSize === size ? "#faf8f5" : "#1a1410", background: selectedSize === size ? "#1a1410" : "transparent", border: `1px solid ${selectedSize === size ? "#1a1410" : "rgba(26,20,16,0.2)"}`, transition: "all 0.18s" }}>
                  {size}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
            <button onClick={handleAddToCart} style={{ width: "100%", padding: "16px 24px", background: added ? "#3a7a4a" : "#1a1410", color: "#faf8f5", fontSize: 10, letterSpacing: "0.24em", textTransform: "uppercase", fontWeight: 600, border: "none", transition: "background 0.25s" }}>
              {added ? "Added ✓" : "Add to Bag"}
            </button>
            <button style={{ width: "100%", padding: "16px 24px", background: "transparent", color: "#1a1410", fontSize: 10, letterSpacing: "0.24em", textTransform: "uppercase", fontWeight: 600, border: "1px solid rgba(26,20,16,0.3)" }}>
              Buy It Now
            </button>
          </div>
