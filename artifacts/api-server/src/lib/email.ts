...
const BRAND_FROM = `Moi <${process.env.RESEND_FROM_EMAIL ?? "onboarding@resend.dev"}>`;
...
export function buildOrderConfirmationEmail(params: {
  ...
  const { orderNumber, customerName, total, paymentMethod, address, governorate, city } = params;
