...
export function NewsletterSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setErrorMsg("");
    try {
      const res = await fetch(`${getApiOrigin()}/api/newsletter`, {
        method: "POST",
    ...
    <section className="relative overflow-hidden px-6 pb-20 md:pb-28 pt-10 md:pt-14">
      ...
      <div className="mx-auto max-w-5xl relative">
        <motion.div
          ...
          className="relative overflow-hidden border border-[rgba(255,255,255,0.06)] bg-[linear-gradient(135deg,rgba(251,247,241,0.96)_0%,rgba(242,235,226,0.93)_52%,rgba(233,223,210,0.9)_100%)]"
          ...
          <div className="relative px-6 py-10 md:px-12 md:py-14">
            <div className="flex flex-col gap-8 md:gap-10">
              ...
              <div className="max-w-3xl flex flex-col gap-4">
                <p style={{ fontFamily: "Cormorant Garamond, serif", fontSize: "clamp(2rem,5vw,4.5rem)", lineHeight: 0.92, letterSpacing: "-0.04em", fontWeight: 600, color: "#1e1814" }}>
                  Stay in Moi-tion.
                </p>
              ...
              <form onSubmit={handleSubmit} className="max-w-2xl flex flex-col gap-4">
                <div className="flex flex-col gap-4 md:flex-row md:items-center">
                  <div className="flex-1 px-4 py-4 border border-[rgba(30,24,20,0.12)] bg-white/25 backdrop-blur-sm">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-[10px] tracking-[0.28em] uppercase" style={{ color: "#7a6e64" }}>Email address</p>
                        <input
                          type="email"
                          required
                  ...
                  <button
                    type="submit"
                    disabled={status === "loading" || status === "success"}
                    className="relative isolate inline-flex items-center justify-center gap-2 overflow-hidden px-6 py-4 text-[11px] tracking-[0.28em] uppercase font-medium text-white transition-transform duration-300 hover:scale-[1.01] disabled:opacity-60"
                ...
                {(status === "error" || errorMsg) && (
                  <p className="text-[11px] tracking-wide" style={{ color: "#a0522d" }}>{errorMsg}</p>
                )}
                {status === "success" && (
                  <p className="text-[11px] tracking-[0.22em] uppercase" style={{ color: "#7a6e64" }}>You're on the list.</p>
                )}
              </form>
            </div>
          </div>
